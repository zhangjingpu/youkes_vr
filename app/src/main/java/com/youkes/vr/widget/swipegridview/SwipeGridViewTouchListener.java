...
package com.youkes.vr.widget.swipegridview;
...
/**
 * Touch listener impl for the SwipeGridView
 */
public class SwipeGridViewTouchListener implements View.OnTouchListener {

    private static final int DISPLACE_CHOICE = 80;

    private int swipeMode = com.youkes.vr.widget.swipegridview.SwipeGridView.SWIPE_MODE_BOTH;
     ...
     * Constructor
     *
     * @param SwipeGridView  SwipeGridView
     * @param swipeFrontView front view Identifier
     * @param swipeBackView  back view Identifier
     */
    public SwipeGridViewTouchListener(SwipeGridView SwipeGridView, int swipeFrontView, int swipeBackView) {
        this.swipeFrontView = swipeFrontView;
        this.swipeBackView = swipeBackView;
        ViewConfiguration vc = ViewConfiguration.get(SwipeGridView.getContext());
        slop = vc.getScaledTouchSlop();
        minFlingVelocity = vc.getScaledMinimumFlingVelocity();
        maxFlingVelocity = vc.getScaledMaximumFlingVelocity();
        configShortAnimationTime = SwipeGridView.getContext().getResources().getInteger(android.R.integer.config_shortAnimTime);
        animationTime = configShortAnimationTime;
        this.SwipeGridView = SwipeGridView;
    }
    ...
    private void setFrontView(View frontView) {
        ...
        frontView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                SwipeGridView.onClickFrontView(downPosition);
            }
        });
    ...
    private void setBackView(View backView) {
        ...
        backView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                SwipeGridView.onClickBackView(downPosition);
            }
        });
    }
    ...
    protected boolean isSwipeEnabled() {
        return swipeMode != SwipeGridView.SWIPE_MODE_NONE;
    }
    ...
    public void resetItems() {
        if (SwipeGridView.getAdapter() != null) {
            int count = SwipeGridView.getAdapter().getCount();
    ...
    protected void openAnimate(int position) {
        openAnimate(SwipeGridView.getChildAt(position - SwipeGridView.getFirstVisiblePosition()).findViewById(swipeFrontView), position);
    }
    ...
    protected void closeAnimate(int position) {
        closeAnimate(SwipeGridView.getChildAt(position - SwipeGridView.getFirstVisiblePosition()).findViewById(swipeFrontView), position);
    }
    ...
    private void swapChoiceState(int position) {
        ...
        if (lastCount == 0 && count == 1) {
            SwipeGridView.onChoiceStarted();
            closeOpenedItems();
            setActionsTo(SwipeGridView.SWIPE_ACTION_CHOICE);
        }
        if (lastCount == 1 && count == 0) {
            SwipeGridView.onChoiceEnded();
            returnOldActions();
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            SwipeGridView.setItemChecked(position, !lastChecked);
        }
        SwipeGridView.onChoiceChanged(position, !lastChecked);
    ...
    protected void unselectedChoiceStates() {
        int start = SwipeGridView.getFirstVisiblePosition();
        int end = SwipeGridView.getLastVisiblePosition();
        for (int i = 0; i < checked.size(); i++) {
            if (checked.get(i) && i >= start && i <= end) {
                reloadChoiceStateInView(SwipeGridView.getChildAt(i - start).findViewById(swipeFrontView), i);
            }
            checked.set(i, false);
        }
        SwipeGridView.onChoiceEnded();
    ...
    protected int dismiss(int position) {
        int start = SwipeGridView.getFirstVisiblePosition();
        int end = SwipeGridView.getLastVisiblePosition();
        View view = SwipeGridView.getChildAt(position - start);
    ...
    protected int getCountSelected() {
        ...
        Log.d("SwipeGridView", "selected: " + count);
        return count;
    }
    ...
    private void generateAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        Log.d("SwipeGridView", "swap: " + swap + " - swapRight: " + swapRight + " - position: " + position);
                ...
                .setListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if (swap) {
                            ...
                            if (aux) {
                                SwipeGridView.onOpened(position, swapRight);
                                openedRight.set(position, swapRight);
                            } else {
                                SwipeGridView.onClosed(position, openedRight.get(position));
                            }
                        }
                        resetCell();
                    }
    ...
    void closeOpenedItems() {
        if (opened != null) {
            ...
            for (int i = start; i <= end; i++) {
                if (opened.get(i)) {
                    closeAnimate(SwipeGridView.getChildAt(i - start).findViewById(swipeFrontView), i);
                }
            }
        }
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        if (viewWidth < 2) {
            viewWidth = SwipeGridView.getWidth();
        }
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            case MotionEvent.ACTION_DOWN: {
                ...
                int childCount = SwipeGridView.getChildCount();
                int[] listViewCoords = new int[2];
                SwipeGridView.getLocationOnScreen(listViewCoords);
                int x = (int) motionEvent.getRawX() - listViewCoords[0];
                int y = (int) motionEvent.getRawY() - listViewCoords[1];
                View child;
                for (int i = 0; i < childCount; i++) {
                    child = SwipeGridView.getChildAt(i);
                    child.getHitRect(rect);

                    int childPosition = SwipeGridView.getPositionForView(child);

                    // dont allow swiping if this is on the header or footer or IGNORE_ITEM_VIEW_TYPE or enabled is false on the adapter
                    boolean allowSwipe = SwipeGridView.getAdapter().isEnabled(childPosition) && SwipeGridView.getAdapter().getItemViewType(childPosition) >= 0;
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (!opened.get(downPosition)) {
                    if (swipeMode == SwipeGridView.SWIPE_MODE_LEFT && velocityTracker.getXVelocity() > 0) {
                        velocityX = 0;
                    }
                    if (swipeMode == SwipeGridView.SWIPE_MODE_RIGHT && velocityTracker.getXVelocity() < 0) {
                        velocityX = 0;
                    }
                }
                ...
                if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity && velocityY * 2 < velocityX) {
                    ...
                    Log.d("SwipeGridView", "swapRight: " + swapRight + " - swipingRight: " + swipingRight);
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                int changeSwipeMode = SwipeGridView.changeSwipeMode(downPosition);
                if (changeSwipeMode >= 0) {
                    swipeMode = changeSwipeMode;
                }

                if (swipeMode == SwipeGridView.SWIPE_MODE_NONE) {
                    deltaMode = 0;
                } else if (swipeMode != SwipeGridView.SWIPE_MODE_BOTH) {
                    if (opened.get(downPosition)) {
                        if (swipeMode == SwipeGridView.SWIPE_MODE_LEFT && deltaX < 0) {
                            deltaMode = 0;
                        } else if (swipeMode == SwipeGridView.SWIPE_MODE_RIGHT && deltaX > 0) {
                            deltaMode = 0;
                        }
                    } else {
                        if (swipeMode == SwipeGridView.SWIPE_MODE_LEFT && deltaX > 0) {
                ...
                if (deltaMode > slop && swipeCurrentAction == SwipeGridView.SWIPE_ACTION_NONE && velocityY < velocityX) {
                    ...
                    Log.d("SwipeGridView", "deltaX: " + deltaX + " - swipingRight: " + swipingRight);
                    if (opened.get(downPosition)) {
                        SwipeGridView.onStartClose(downPosition, swipingRight);
                        ...
                        SwipeGridView.onStartOpen(downPosition, swipeCurrentAction, swipingRight);
                    }
                    SwipeGridView.requestDisallowInterceptTouchEvent(true);
                    MotionEvent cancelEvent = MotionEvent.obtain(motionEvent);
                    cancelEvent.setAction(MotionEvent.ACTION_CANCEL |
                            (MotionEventCompat.getActionIndex(motionEvent) << MotionEventCompat.ACTION_POINTER_INDEX_SHIFT));
                    SwipeGridView.onTouchEvent(cancelEvent);
                    if (swipeCurrentAction == SwipeGridView.SWIPE_ACTION_CHOICE) {
    ...
    public void move(float deltaX) {
        SwipeGridView.onMove(downPosition, deltaX);
        ...
        if (posX > 0 && !swipingRight) {
            Log.d("SwipeGridView", "change to right");
        ...
        if (posX < 0 && swipingRight) {
            Log.d("SwipeGridView", "change to left");
            swipingRight = !swipingRight;
    ...
    private void removePendingDismisses(int originalHeight) {
        ...
        SwipeGridView.onDismiss(dismissPositions);
